...
Condition* cond[MAX_COND];
int cond_key[MAX_COND] = {0};
...
ExceptionHandler(ExceptionType which)
{
	...
	DEBUG('a', "Shutdown, initiated by user program.\n");
      ...
      int key = machine->ReadRegister(4);
      bool found=0;
      for(int j=0;j<MAX_COND && !found;j++){
        if(cond_key[j]==key){
          machine->WriteRegister(2,j);
          found=1;
          break;
      ...
      if(cond_key[id]==0 || sem_key[semid]==0){
        machine->WriteRegister(2,-1);
      }
      else if(op == COND_OP_WAIT){
        cond[id]->Wait(sem[semid]);
        machine->WriteRegister(2,0);
      }
